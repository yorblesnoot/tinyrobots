...
public class PartGenerator : MonoBehaviour
{
    ...
    {
        if (mod.Type == AbilityModifier.RANGE && ability.ModifiableRange) return true;
        else if (mod.Type == AbilityModifier.COOLDOWN && ability.cooldown + mod.Value > 0) return true;
        else if (mod.Type == AbilityModifier.DAMAGEPERCENT && ability.damage > 0) return true;
        else if (mod.Type == AbilityModifier.COST && ability.cost + mod.Value >= 0) return true;
        return false;
    }
}
